# EKS kube-proxy conntrack 분석 에이전트

## 역할

EKS 1.35에서 kube-proxy 모드별(iptables/ipvs/nftables) conntrack table sync 시간 영향도를 비교 분석하는 프로젝트의 AI 어시스턴트.
...
├── docs/                # 아키텍처 문서, ADR, Runbook, 설계 스펙
└── tools/               # 유틸리티 스크립트, 프롬프트
```

## 실행 순서

1. `./01.create-clusters.sh [iptables|ipvs|nftables|all]` — EKS 클러스터 생성
...
| ekscluster01-ipvs | ipvs (scheduler: rr) | m6g.xlarge × 4 |
...
- `kubeproxy_sync_full_proxy_rules_duration_seconds` — full resync 시간
...
- conntrack-monitor는 `/host/proc/sys`로 마운트
